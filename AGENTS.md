...
7. **Name the background docs in the brief (lead's job).** Only `CLAUDE.md` loads automatically on spawn — a teammate with a narrow task will not go read 200 lines of history on a hunch. When a task touches ground that has been walked before, the dispatch prompt cites the specific doc *and what to look for in it*: `docs/CHANGELOG-implementation.md` (grep by model id / task id / bundle name — it has an Index), `model-integration-precheck.md` (mandatory for any new model/entity/output-grid registration), the feature's `docs/superpowers/specs|plans/` pair, or `docs/superpowers/HARNESS.md`. "Read `CLAUDE.md`" is not a citation — it is already loaded. `CLAUDE.md`'s "Where things live" table is the routing list.
8. **History goes in the changelog, not `CLAUDE.md`** (hard rule #9). A teammate's task record — commit SHA, gate counts, deviations — is appended to `docs/CHANGELOG-implementation.md` in the same commit as the work. `CLAUDE.md` changes only when a *durable* rule or gotcha is discovered, and then only the distilled rule.
