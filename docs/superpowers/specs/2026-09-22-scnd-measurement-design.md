...
- Declare warm-up and outlier policy. Report **raw rows** for everything, and **uncertainty (bootstrap CI) on the four decision metrics**: mean CPU service demand, p95 wall, failure rate, and the paired objective delta. p50, RSS and corpus frequency are descriptive point estimates backed by the raw rows.
  > **Revised 2026-09-23 (plan review L-R5, applied to the spec in round 6).** This bullet previously demanded uncertainty on p50/p95, mean service demand, slow-regime frequency, failure/rejection rates, RSS *and* objective delta. The plan's accepted scope reduction limits CIs to the four metrics that can change the sizing decision; a CI on a descriptive quantity is cost without evidence. The plan and spec contradicted each other on this until now; the plan's settled decision is the one kept.
...
> **Revised 2026-09-23 (plan review L-R5, applied to the spec in round 6).** Of the list above, **CPU throttling, network errors and three-hour storage growth are optional supporting diagnostics**, not mandatory captures and never blockers on a verdict — Render exposes no direct throttled-time metric, and the plan's M3.4a mandatory core (offered/achieved rate, the three latencies, rejection/failure/timeout, queue depth, active solve count, instance CPU, aggregate RSS/OOM/restarts, event-loop lag, pool wait, query latency, lock delay) is sufficient to name the binding constraint. The spec listed them as captured while the plan had demoted them; the plan's settled decision is the one kept.
