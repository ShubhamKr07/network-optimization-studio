...
**Out of scope (other specs):** worker split / scheduler / topology / single-flight / retention (→ Scaling, `2026-09-22-scnd-scaling-design.md`); benchmark, MIP-start, warm-worker, plan matrix + topology comparison (→ measurement); Quick mode (→ later). **Rerouted INTO Option A** (`plans/2026-09-22-scnd-correctness-A-full-contract.md`, A-R4/A-R17): **durable queue + restart + owner-lease recovery** (A1/A2) and the **per-scenario stale-result publication CAS** (A7). **Single-flight** was briefly A10 and has been **returned to Scaling** (see the A10 removal record); worker split / scheduler / topology / retention likewise stay with Scaling. The stale `(→ B2)` routing of these items is withdrawn — "B2" is superseded by the A/B split.
...
> **SUPERSEDED IN PART (2026-09-22, review A-R4).** This section was written before the A/B split and assumes R1's writer emits **v1**. Post-Option-B that premise is false: B ships a **third** representation — truthful `solutionStatus`/`terminationReason` with no `envelopeVersion` and no five-schema contract. The corrected R1 writes **B-format** behind a **three-way** reader (legacy / B-unversioned / v2). Task **A0** of `plans/2026-09-22-scnd-correctness-A-full-contract.md` owns the representation inventory; that inventory and the authoritative release-state matrix are now recorded in **§2.14** below (five representations; R1/R2/R3/rollback/cleanup states). **Read §2.14 first** — the named releases below are reconciled to it (R1 = three-way reader + B-format writes, not "dual reader" / "v1"). The rollback floor, the `v2_write` flag, the drain-proof requirement and the permanent legacy-reader retention (Q69) are unchanged.
...
Named releases:
1. **R1 (schema + three-way reader):** nullable `solve_jobs` column push; deploy an API whose new readers **explicitly discriminate** the three stored representations — historical unversioned legacy / B truthful-but-unversioned / v2 (§2.14); **public responses stay old-client-compatible**; **writer continues to emit B-format** (truthful `solutionStatus`/`terminationReason`, no `envelopeVersion`) — **not** v1.
...
3. **R3 (writer, flagged):** after **every pre-three-way-reader API instance drains** (proven from Render deploy/instance/log evidence — the platform's own drain record after `maxShutdownDelaySeconds + margin`, per A11/A-R46; the version signal is corroboration only, not proof), enable the **default-off** `v2_write` server flag (owner: API); v2 writes begin.
...
Closed items: flag = `v2_write` (owner: API, default off; exact key `SOLVER_V2_WRITE_ENABLED`, A11); drain-proof = Render deploy/instance/log evidence that the prior revision drained (A-R46), **not** health sampling; tests = **R3-writer/R1-three-way-reader** (a pre-R1 reader is forbidden after the first v2 write — the old `new-writer/old-reader` test is invalid); deploy order `nos-api`→`nos-studio`; compatibility-window exit = observed client versions past the cutoff.

### 2.14 Post-B representation inventory + release-state matrix (A0; Q73/Q76/Q80/Q81)

**The five representations.** Post-Option-B there are exactly five result representations in play; naming them is the precondition for the three-way reader (§2.13/R1) and the staged rollout:

1. **Historical unversioned legacy rows** — the pre-B `_envelope` shape (`{status, objective, runTimeSec, quality, edges, metrics, details, solverUsed, infeasibilityReason}`), **no** `envelopeVersion`, **no** `solutionStatus`/`terminationReason`. Read via the normalizer → normalized v1 (`envelopeVersion:1`, `solutionStatus:null`, `terminationReason:"unknown"`, `legacyUnverified:true`, §2.7); **never** promoted to proven.
2. **B's truthful-but-unversioned rows** — Option B added truthful `solutionStatus`/`terminationReason` (+ `achievedGap`/`solverIncumbentObjective`/`solverBestBound`) but **no** `envelopeVersion` and **no** five-schema contract. This is the **third** representation the pre-split §2.13 did not anticipate (A-R4).
3. **Canonical A v2 published results** — `PublishedSolveResultV2` (§2.6): `envelopeVersion:2`, full field set incl. the §2.12 limit fields, `legacyUnverified:false`. The **only** shape that reaches `scenarios.result`/public APIs, and only under R3.
4. **Existing v1 cache entries** — `result_cache` rows keyed on the `solve.py`-only `SOLVER_CODE_HASH` (pre-A6). Under A6's composite identity they are a **cache miss** — read but never trusted as v2.
5. **Composite-versioned v2 cache entries** — `ResultCacheEntryV2` (§2.6) keyed on A6's composite solver-contract identity (effective-limit/version key); written only after A6 lands and R3 is active.

**Decided baseline (A-R4).** **R1** = a **three-way** stored-result reader (legacy / B-unversioned / v2) that continues **B-format** writes. **R2** = a client compatible with **both** B and A v2. **R3** = versioned (v2) writes **only after drain proof**. **Q80 (decided):** the permanent failed-job API is **`errorCode` + a permanent `errorMessage`** (server-owned safe message derived from §2.11's table); `errorMessage` is **not** transitional — only the *old raw* `SolveJob.error` alias is removed at cleanup.

**Release-state matrix** (dimensions × release states):

| Dimension | R1 (schema + three-way reader) | R2 (frontend) | R3 (writer, flagged) | Rollback (floor = R1) | Cleanup |
|---|---|---|---|---|---|
| **Stored-result reader** | three-way: legacy-normalizer / B-unversioned / v2 | same three-way (backend unchanged) | three-way (retained) | three-way (never below R1 once any v2 row exists) | three-way retained **indefinitely** (Q69) |
| **Writer** | **B-format** (truthful `solutionStatus`/`terminationReason`, no `envelopeVersion`) | B-format | **v2** (`PublishedSolveResultV2`) | v2 disabled → reverts to B-format | v2 only |
| **Public serializer** | old-client-compatible (new fields absent/optional; gated separately from writes) | adds `errorCode`/`errorMessage` **alongside** old `error` for the window | v2 fields present; `error` alias still served | old-client-compatible | transitional `error` alias **removed** (first release after threshold); `errorCode`/`errorMessage` **permanent** (Q80) |
| **Cache reader** | v1 (`SOLVER_CODE_HASH`) | v1 | v2 (composite identity, A6); v1/unversioned = **miss** | v1 + v2 readable; unversioned = miss | v2 (composite identity) |
| **Cache writer** | v1 (`SOLVER_CODE_HASH`) — no v2 cache until A6 | v1 | v2 (composite identity) | v1 (v2 disabled) | v2 |
| **Frontend behavior** | unchanged; tolerant of B fields | compatible with **both** B and v2; consumes `errorCode`/`errorMessage` + retry affordance | renders nullable-`status` union, legacy badges, `errorCode`/`errorMessage` | R2 client tolerates both | v2 only; legacy badges retained for historical rows |
| **Flag state** (`SOLVER_V2_WRITE_ENABLED`) | `false` (default off) | `false` | **`true`** (product-owner env flip; external Render change, **not** a commit) | `false` (disabled **before** rollback) | `true` (stable) |
| **Deploy ordering** | `nos-api` first | `nos-studio` after `nos-api` | prerequisites complete → `nos-api` → `nos-studio` | roll `nos-api` back to the R1 revision | normal |
| **Drain evidence** | — (R1 write behavior does not depend on it) | n/a (frontend) | platform deploy/instance/log evidence prior revision drained ≥ `maxShutdownDelaySeconds + margin` (**180 s**, A14a); version signal is corroboration only | n/a | n/a |
| **Compatibility evidence** | three-way reader explicitly discriminates legacy/B/v2 | ≥ **7-day** window, **zero** requests from a pre-R2 client build | approved G-cache artifact + committed/reviewed A13a evidence + Linux no-orphan proof (A3) | three-way reader handles v2 rows written pre-rollback | observed zero pre-R2 clients across the window |
| **Treatment of already-written rows** | legacy + B rows read via three-way; **never rewritten/backfilled** | unchanged | legacy/B rows still read via three-way; no backfill | already-written v2 `scenarios.result`/`solve_jobs.result`/v2 cache rows **left in place and readable**; nothing rewritten or deleted; unversioned cache rows stay a miss | only the **old raw `SolveJob.error` alias** removed (Q80); unversioned legacy stored rows persist forever, still read via the normalizer |
...
### P0R.2 — fixtures + parser tests — ✅ DONE in part (delivered with P0R.1, 32 tests pass); jobRunner process-level portion OPEN → A3
Category status: the parser, fixture, and Python-wrapper categories (1–3 below) are **complete**. The **process-level `jobRunner` category (4 below) is OPEN** and owned by **A3** (fd3 protocol + Node process-group supervisor + terminal state machine) — the fd3-transport and process-supervision tests cannot exist until A3's runner lands.
...
4. **Process-level `jobRunner` failure tests** (**OPEN → owned by A3**) — missing exe, nonzero exit, malformed stdout, parser exception, cleanup failure, outer timeout.
- **Error boundary (per canonical §2.1/§2.4/§2.11 — Q60=A/Q37):** there is **no error envelope anywhere** — `error` is **not** a `solutionStatus` and `solver_error` is **not** a `terminationReason`. Every execution failure is a **failed `SolveJob`** carrying a public `errorCode` (`SOLVE_FAILED`/`TIMEOUT`) + a server-owned safe `errorMessage`, with **no scenario result published and nothing cached**; the granular cause (`data_error`/`model_error`/`internal_error`/`solver_error`) is the **internal `failureReason`** (§2.11), never public (cached: no, published: no). Failures *before/outside* CBC (spawn/timeout/nonzero-exit/JSON/schema) → the same **failed job with no published scenario result**. Do **not** fabricate CBC artifacts for non-CBC failures. Fixtures per category (§20.2.7): attainable-CBC / synthetic-parser / wrapper / jobRunner-process + negative-leakage assertions (§2.11).
...
### P0R.3 — contract + schemas + OpenAPI + frontend + policy + rollout (NOT approved: §34 controls — blocked until Q73–Q84 + the §34 checklist close; then conditional on the consolidated post-spike review). Every §27/§28 decision has a named task/seam/test (§28.2.4/Q53):
...
### P0R.4 — integration tests + DEC-2026-09-21-01 correction + e2e_journey repair + full gate (NOT approved: §34 controls — blocked until Q73–Q84 + the §34 checklist close)
...
| ID | Task | Gate |
|---|---|---|
| P0R.1 | CBC evidence spike (wrapper+temp-dir primary; direct-CBC fallback needs no-go) | **✅ EXECUTED — GO (2026-09-22)** |
| P0R.2 | Fixture capture + parser tests | ✅ DONE (parser/fixtures/Python-wrapper, 32 tests); **jobRunner process-level portion open → A3** |
| P0R.3 | v2 contract + **five schemas** (`SolverSuccessEnvelopeV2`/`ResultCacheEntryV2`/`PublishedSolveResultV2`/`StoredScenarioResult`/`NormalizedSolveResult`, §2.6) + OpenAPI/regen + normalizer wiring + frontend + policy branch + `_envelope_compat.py` | **NOT approved** until Q73–Q84 + the §34 checklist close (§34 controls); then conditional on the consolidated post-spike review |
| P0R.4 | Integration tests + DEC-2026-09-21-01 correction + `e2e_journey.py` repair + both standalone scripts + full gate | **NOT approved** until Q73–Q84 + the §34 checklist close (§34 controls) |
