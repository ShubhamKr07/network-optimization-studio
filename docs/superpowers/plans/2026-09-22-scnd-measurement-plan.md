...
> **Locked-model measurement (follow-up, added 2026-09-24 per product-owner instruction).** Locked chapters (`capabilities.locked` → 403 on every scenario route: jade Ch9, chens Ch4 today, and **any newly-integrated locked model**) cannot be exercised over the student-facing HTTP API, so their solve profiles are silently absent from an HTTP load run. In the first isolated-env campaign (2026-09-24) the all-JADE profile was **dropped** for exactly this reason (MP-1 decision), which under-weights the heavy (~13s) tail vs the full-corpus capacity model. **Any future measurement campaign that must represent a locked model MUST add an explicit step here to make it measurable on the isolated env — either unlock it via a measure-only application SHA (documented as diverging from prod parity) or add a measurement seam — rather than dropping it unnoticed.** Do not treat a locked model's absence as "0 load"; state it.
