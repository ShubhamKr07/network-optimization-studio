...
interface WarehouseTableProps {
  /** T8 (Workspace fixups 2, item 3) — the rows to render, ALREADY filtered
   * by the caller when it has a FilterMenu (mirrors CustomerTable/
   * CustomersTab's established split: this component owns display only, the
   * base tab owns filtering + the FilterMenu, so the Filter control can sit
   * on the tab's own Import/Export toolbar row instead of an orphaned row
   * here). Callers with no filter concept (Studio.tsx's Overrides dialog)
   * just pass the full array, unaffected by this split. */
