...
router.post("/auth/register", async (req: Request, res: Response) => {
  ...
  posthog?.identify({ distinctId: user.id, properties: { email: user.email, role: user.role } });
  posthog?.capture({ distinctId: user.id, event: "user registered", properties: { role: user.role } });
