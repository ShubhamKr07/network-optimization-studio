...
describe("twoEchelonInputsSchema — distanceBands relaxed to positive numbers (T3b)", () => {
  it("accepts a non-integral band produced by a unit conversion", () => {
    expect(() =>
      twoEchelonInputsSchema.parse({ ...BASE, distanceBands: [310.6856, 621.3712] })
    ).not.toThrow();
  });

  it("still rejects zero and negative bands", () => {
    expect(() => twoEchelonInputsSchema.parse({ ...BASE, distanceBands: [0] })).toThrow();
    expect(() => twoEchelonInputsSchema.parse({ ...BASE, distanceBands: [-5] })).toThrow();
  });
  ...
  it("still ACCEPTS duplicate / non-ascending arrays, exactly as before", () => {
    expect(() =>
      twoEchelonInputsSchema.parse({ ...BASE, distanceBands: [400, 200, 400] })
    ).not.toThrow();
  });
});
