...
- [x] A3.2 — Remove gamification (frontend) + new auth pages. Deleted `pages/arcadia/`, `GamificationContext`, `ArcadiaShell`; new `pages/auth/{Login,Register}.tsx` on the generated auth hooks + a plain `AppShell`. `App.tsx` gates on `useGetCurrentAuthUser`. Studio's "which lab" state is now local (`activeModelIndex`), not context-backed. Commit `44cb9a6`. **Known gap**: `e2e/labs.spec.ts` was rewritten for the new auth/no-QuestMap reality but not executed — no wired-up local same-origin dev setup (frontend/backend on separate ports, no dev proxy) and the default `E2E_BASE_URL` targets a stale Replit deployment. Needs either a local proxy + two dev servers running, or a fresh Replit deploy, before it can actually be run.
- [ ] B1.1 — Chapter landing + routes — **next up**.
