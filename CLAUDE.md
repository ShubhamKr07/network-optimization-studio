...
- [ ] D1.1 — solver override capability (per-warehouse capacity, demand overrides, exclusions actually wired into solve.py; `warehouseOverrides`/`customerOverrides` are already structurally validated and round-tripped, but the solver only honors `.status` today, not `.capacity`/`.demand`) — not started.
- [ ] D1.2 — left panel rework — not started.
- [ ] D2.1/D3.1 — warehouse/customer table components — not started.
- [ ] D4.1 — export — not started.
- [ ] D5.1/D5.2 — import parse/validate/preview + UI — not started.
- [ ] D6.1 — reset to baseline — not started.
- [ ] X1.1 — staleness guard — not started.
...
**Local dev DB:** no `DATABASE_URL` in the environment by default; a local Postgres 18 is running with a `nos_dev` database matching the current schema (post-D0.2: `model_id`/`inputs` jsonb, no more flat per-model columns). Pass `DATABASE_URL="postgresql://shubhamkr@localhost:5432/nos_dev"` inline per command (shell env doesn't persist across tool calls in this session).
